...
/**
 * 大問のグループキーを取得
 * - 大問1: 各問題が独立（計算問題のため）→ 問題IDをキーにして1問1グループ
 * - 大問2以降: 小問番号を除いた大問単位でグループ化
 *   "第405回公開テスト 大問4(1)" → "第405回公開テスト 大問4"
 */
export function getGroupKey(problem) {
  const base = problem.source.replace(/[\(（][\d]+[\)）]$/, '').trim();
  // 大問1は各問題が独立した計算問題なので、1問ずつ独立グループ
  if (/大問1$/.test(base)) {
    return problem.id;
  }
  return base;
 ...
 * - 大問2以降の同じ大問内の小問は元の順序を維持
 * - 大問1の各問題は独立扱いで他の大問と均等にシャッフル
 */
