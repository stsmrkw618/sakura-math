...
**ユーザーがテスト写真から手動でトリミングした画像を使用する。** Claude Codeは図形のトリミングを行わない。

### `figure` フィールドの仕様

問題データの `figure` フィールドは `null`（図なし）または以下:
...
```
- ユーザーがテスト写真から該当部分をトリミングし、画像を添付する
- Claude Codeは受け取った画像をPNG変換して `public/figures/` に保存
- `path` は `public/` からの相対パス（Next.jsの静的ファイル配信）
- `description` は図の説明文（アクセシビリティ用）
...
問題登録時の1問ずつレビューで:
  ↓
Claude Code: 「この問題に図形はありますか？」
  ↓
┌─ 図なし → figure: null
│
└─ 図あり → ユーザーが手動トリミングした画像を添付
    → Claude Codeが画像をPNG変換して public/figures/{問題ID}.png に保存
    → figure: { type: "image", path: "figures/{問題ID}.png", description: "..." }
    → description はClaude Codeが画像を見て生成
```
...
4. 答えと解き方を生成（模範解答）
5. IDを自動採番（"YYYY-MM-NNN" 形式）
6. **1問ずつレビュー**: 問題文・答え・図形の有無をユーザーに確認
   - 図形がある問題: ユーザーにトリミング画像の添付を依頼 → 受け取った画像を `public/figures/` に保存
   - 図形がない問題: `figure: null`
7. 全問OKの後、`data/problems.json` に追記

詳細な手順は `scripts/register-from-photo.md` を参照。
