  ...
  return (
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-3">
        <span className={`px-3 py-1 rounded-full text-sm font-bold ${catColor.bg} ${catColor.text} ${catColor.border} border`}>
          {category.emoji} {category.name}
        </span>
        <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${boxColor}`}>
          {boxLabel}
        </span>
      </div>
        ...
        >
          ...
          <div className="flip-card-back absolute inset-0 w-full">
            <div className="w-full h-full bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl border-2 border-purple-200 shadow-lg p-6 flex flex-col items-center justify-center">
              <p className="text-3xl font-bold text-purple-700 font-kiwi text-center leading-relaxed whitespace-pre-line">
                {card.back}
              </p>
              {card.hint && (
