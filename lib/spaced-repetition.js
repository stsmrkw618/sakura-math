...
export function getDueProblems(problems, reviews, options = {}) {
  ...
  // 逆引きマップ: この問題に依存する問題（大問の後続）
  const dependentsMap = {};
  problems.forEach((p) => {
    if (p.prerequisites) {
      p.prerequisites.forEach((prereqId) => {
        if (!dependentsMap[prereqId]) dependentsMap[prereqId] = [];
        dependentsMap[prereqId].push(p);
      });
    }
  });
  ...
  const buildWithPrereqs = (list) => {
    ...
    const addProblem = (problem) => {
      if (addedIds.has(problem.id)) return;
      // 前提問題を先に追加（助走）
      if (problem.prerequisites) {
        for (const prereqId of problem.prerequisites) {
          if (problemMap[prereqId]) addProblem(problemMap[prereqId]);
        }
      }
      // 本問を追加
      addedIds.add(problem.id);
      result.push(problem);
      // 依存する後続問題も追加（大問グループを完結させる）
      if (dependentsMap[problem.id]) {
        for (const dep of dependentsMap[problem.id]) {
          addProblem(dep);
        }
      }
    };
