...
export function loadProgress() {
  ...
  try {
    ...
    // マイグレーション: customCards/hiddenCards がない旧データ対応
    if (!parsed.flashcards.customCards) {
      parsed.flashcards.customCards = [];
    }
    if (!parsed.flashcards.hiddenCards) {
      parsed.flashcards.hiddenCards = [];
    }

    // マイグレーション: 旧 sakura_custom_flashcards からの移行
    const oldCustomKey = 'sakura_custom_flashcards';
    const oldCustomRaw = localStorage.getItem(oldCustomKey);
    if (oldCustomRaw) {
      try {
        const oldCustom = JSON.parse(oldCustomRaw);
        const oldAdded = oldCustom.added || [];
        const oldHidden = oldCustom.hidden || oldCustom.deleted || [];
        // 既存に無いものだけ追加
        const existingIds = new Set(parsed.flashcards.customCards.map(c => c.id));
        for (const card of oldAdded) {
          if (!existingIds.has(card.id)) {
            parsed.flashcards.customCards.push(card);
          }
        }
        const existingHidden = new Set(parsed.flashcards.hiddenCards);
        for (const id of oldHidden) {
          if (!existingHidden.has(id)) {
            parsed.flashcards.hiddenCards.push(id);
          }
        }
...
function mergeProgress(local, remote) {
  ...
  if (remote.flashcards) {
    ...
    // customCards: 両方のカードをIDベースでマージ
    const lCustom = local.flashcards?.customCards || [];
    const rCustom = remote.flashcards?.customCards || [];
    const customMap = new Map();
    for (const c of lCustom) customMap.set(c.id, c);
    for (const c of rCustom) { if (!customMap.has(c.id)) customMap.set(c.id, c); }
    merged.flashcards.customCards = [...customMap.values()];
    // hiddenCards: 両方の和集合
    const hiddenSet = new Set([
      ...(local.flashcards?.hiddenCards || []),
      ...(remote.flashcards?.hiddenCards || []),
    ]);
    merged.flashcards.hiddenCards = [...hiddenSet];
  }
